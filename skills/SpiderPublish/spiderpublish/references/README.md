...
| [`directory-pages.md`](directory-pages.md) | Building a programmatic-SEO **directory** — categories + per-city pages + listing details, generated from the tenant's own business data. The copy-not-IDAP model, the four-level URL shape, the renamable `{directory_base}` prefix, server-side SEO templates that also feed `sitemap.xml`, why re-import never removes anything and what `prune` does about it, the three re-import doors (MCP / CLI / dashboard), and the three overridable Liquid templates. |
| [`dynamic-collections.md`](dynamic-collections.md) | Picking a `collection_type` for a `dynamic_list` / `dynamic_item` page. All five accepted values with honest status (`posts`, `docs`, `directory_listings`, `idap.<collection>`, a custom-collection slug), the item-URL per type, the authored-vs-default render modes, why a bad binding is a **201 that renders an empty page forever**, and why merge tags never populate here. |
